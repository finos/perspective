...
namespace compute {
    ...
    /// \brief A mutable central function registry for built-in functions as
    /// well as user-defined functions. Functions are implementations of
    /// arrow::compute::Function.
    ///
    /// Generally, each function contains kernels which are implementations of a
    /// function for a specific argument signature. After looking up a function
    /// in the registry, one can either execute it eagerly with
    /// Function::Execute or use one of the function's dispatch methods to pick
    /// a suitable kernel for lower-level function execution.
    class ARROW_EXPORT FunctionRegistry {
    public:
        ~FunctionRegistry();

        /// \brief Construct a new registry. Most users only need to use the
        /// global registry
        static std::unique_ptr<FunctionRegistry> Make();

        /// \brief Add a new function to the registry. Returns Status::KeyError
        /// if a function with the same name is already registered
        Status AddFunction(
            std::shared_ptr<Function> function, bool allow_overwrite = false
        );

        /// \brief Add aliases for the given function name. Returns
        /// Status::KeyError if the function with the given name is not
        /// registered
        Status AddAlias(
            const std::string& target_name, const std::string& source_name
        );

        /// \brief Add a new function options type to the registry. Returns
        /// Status::KeyError if a function options type with the same name is
        /// already registered
        ...
        GetFunctionOptionsType(const std::string& name) const;
