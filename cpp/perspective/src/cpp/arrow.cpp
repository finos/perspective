...
namespace arrow {
    ...
    ArrowLoader::initialize(const uintptr_t ptr, const uint32_t length) {
        ...
        if (std::memcmp("ARROW1", (const void*)ptr, 6) == 0) {
            ...
            ::arrow::Status status = ipc::RecordBatchFileReader::Open(
            ...
            ::arrow::Status status = ipc::RecordBatchStreamReader::Open(
                &buffer_reader, &batch_reader);
            if (!status.ok()) {
                std::cerr << status.message() << std::endl;
                PSP_COMPLAIN_AND_ABORT(status.message());
    ...
    void
    ArrowLoader::fill_table(t_data_table& tbl, const std::string& index,
        ...
        switch (src->type()->id()) {
            case ::arrow::DictionaryType::type_id: {
                ...
                switch (indices->type()->id()) {
                    ...
                    default:
                        std::stringstream ss;
                        ss << "Could not copy Arrow column of type '"
            ...
            case ::arrow::BooleanType::type_id: {
                auto scol
                    = std::static_pointer_cast<::arrow::BooleanArray>(src);
            ...
            default: {
                PSP_COMPLAIN_AND_ABORT(
                    "Could not copy Arrow column of unknown type.");
            }
        }
    }
