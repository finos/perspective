...
public:
    Status
    AddFunction(std::shared_ptr<Function> function, bool allow_overwrite) {
        ...
        if (it != name_to_function_.end() && !allow_overwrite) {
            return Status::KeyError(
                "Already have a function registered with name: ", name
            );
        }
    ...
    Status
    AddAlias(const std::string& target_name, const std::string& source_name) {
        std::lock_guard<std::mutex> mutation_guard(lock_);

        auto it = name_to_function_.find(source_name);
        if (it == name_to_function_.end()) {
            return Status::KeyError(
                "No function registered with name: ", source_name
            );
        }
    ...
    AddFunctionOptionsType(
        const FunctionOptionsType* options_type, bool allow_overwrite = false
    ) {
        std::lock_guard<std::mutex> mutation_guard(lock_);
    ...
    GetFunctionOptionsType(const std::string& name) const {
        ...
        if (it == name_to_options_type_.end()) {
            return Status::KeyError(
                "No function options type registered with name: ", name
            );
        }
        return it->second;
    }
...
std::unique_ptr<FunctionRegistry>
...
FunctionRegistry::AddFunction(
    std::shared_ptr<Function> function, bool allow_overwrite
) {
    return impl_->AddFunction(std::move(function), allow_overwrite);
}

Status
FunctionRegistry::AddAlias(
...
Status
FunctionRegistry::AddFunctionOptionsType(
...
Result<std::shared_ptr<Function>>
FunctionRegistry::GetFunction(const std::string& name) const {
...
Result<const FunctionOptionsType*>
FunctionRegistry::GetFunctionOptionsType(const std::string& name) const {
    return impl_->GetFunctionOptionsType(name);
}

int
FunctionRegistry::num_functions() const {
    return impl_->num_functions();
}
