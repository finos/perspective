    ...
    namespace {
        ...
        InvalidMessageType(MessageType expected, MessageType actual) {
            return Status::IOError(
                "Expected IPC message of type ",
                FormatMessageType(expected),
                " but got ",
...
#define CHECK_HAS_BODY(message)                                                \
    do {                                                                       \
        if ((message).body() == nullptr) {                                     \
            return Status::IOError(                                            \
                "Expected body in IPC message of type ",                       \
...
#define CHECK_HAS_NO_BODY(message)                                             \
    do {                                                                       \
        if ((message).body_length() != 0) {                                    \
            return Status::IOError(                                            \
                "Unexpected body in IPC message of type ",                     \
                FormatMessageType((message).type())                            \
            );                                                                 \
        }                                                                      \
    } while (0)
    ...
    public:
        ...
        LoadCommon(Type::type type_id) {
            ...
            if (internal::HasValidityBitmap(type_id, metadata_version_)) {
                ...
                if (out_->null_count != 0) {
                    RETURN_NOT_OK(GetBuffer(buffer_index_, out_->buffers.data())
                    );
                }
                buffer_index_++;
            }
        ...
        template <typename T>
        enable_if_t<
            std::is_base_of_v<FixedWidthType, T>
        ...
        Visit(const UnionType& type) {
            ...
            if (out_->null_count != 0 && out_->buffers[0] != nullptr) {
                return Status::Invalid(
                    "Cannot read pre-1.0.0 Union array with "
    ...
    LoadRecordBatchSubset(
        ...
        if (inclusion_mask != nullptr) {
            filtered_schema =
                ::arrow::schema(std::move(filtered_fields), schema->metadata());
            columns.clear();
        } else {
    ...
    ReadRecordBatch(
        ...
        return ReadRecordBatch(
            *message->metadata(), schema, dictionary_memo, options, reader.get()
        );
    }
    ...
    ReadRecordBatchInternal(
        ...
        if (batch == nullptr) {
            return Status::IOError(
                "Header-type of flatbuffer-encoded Message "
                "is not RecordBatch."
            );
        }
    ...
    GetInclusionMaskAndOutSchema(
        const std::shared_ptr<Schema>& full_schema,
        ...
        *out_schema = schema(
            std::move(included_fields),
            full_schema->endianness(),
    ...
    UnpackSchemaMessage(
        ...
        RETURN_NOT_OK(GetInclusionMaskAndOutSchema(
            *schema, options.included_fields, field_inclusion_mask, out_schema
        ));
    ...
    public:
        ...
        ReadNext() override {
            ...
            return ReadRecordBatchInternal(
                *message->metadata(),
                schema_,
                field_inclusion_mask_,
                context,
    ...
    public:
        ...
        static Result<std::shared_ptr<RecordBatch>>
        ReadRecordBatch(RecordBatchFileReaderImpl* state, Message* message);
    ...
    public:
        RecordBatchFileReaderImpl() = default;
        ...
        ReadRecordBatchAsync(int i) {
            ...
            return Status::Invalid(
                "Asynchronous record batch reading is only "
        ...
        ReadRecordBatchWithCustomMetadata(int i) override {
            ...
            if (cached_metadata != cached_metadata_.end()) {
                auto result =
                    ReadCachedRecordBatch(i, cached_metadata->second).result();
        ...
        CountRows() override {
            ...
            for (int i = 0; i < num_record_batches(); i++) {
                ...
                if (batch == nullptr) {
                    return Status::IOError(
                        "Header-type of flatbuffer-encoded "
        ...
        Open(
            ...
            if (!metadata_cache_) {
                metadata_cache_ =
                    std::make_shared<io::internal::ReadRangeCache>(
                        file,
                        file->io_context(),
                        options.pre_buffer_cache_options
                    );
            }
            file_ = file;
            options_ = options;
            ...
            return ReadFooterAsync(cpu_executor)
                .Then([self, options]() -> Status {
                    ...
                    RETURN_NOT_OK(UnpackSchemaMessage(
                        self->footer_->schema(),
                        options,
                        &self->dictionary_memo_,
                        &self->schema_,
                        &self->out_schema_,
                        &self->field_inclusion_mask_,
                        &self->swap_endian_
                    ));
        ...
        GetRecordBatchGenerator(
            ...
            return WholeIpcFileRecordBatchGenerator(
                std::move(state), std::move(cached_source), io_context, executor
            );
        }
    ...
    private:
        ...
        void
        AddMetadataRanges(
            const std::vector<int>& indices, std::vector<io::ReadRange>* ranges
        ) {
            for (int index : indices) {
        ...
        GetIpcReadContext(
            const flatbuf::Message* message, const flatbuf::RecordBatch* batch
        ) {
            IpcReadContext context(&dictionary_memo_, options_, swap_endian_);
            Compression::type compression;
            RETURN_NOT_OK(GetCompression(batch, &compression));
        ...
        Result<const flatbuf::RecordBatch*>
        ...
        struct CachedRecordBatchReadContext {
            CachedRecordBatchReadContext(
                ...
                cache(
                    file, file->io_context(), io::CacheOptions::LazyDefaults()
                ),
                length(batch->length()) {}
        ...
        ReadCachedRecordBatch(
            ...
            return dictionary_load_finished_
                ...
                .Then(
                    [this, index](const std::shared_ptr<Message>& message_obj
                    ) -> Future<std::shared_ptr<RecordBatch>> {
                        FileBlock block = GetRecordBatchBlock(index);
                        ARROW_ASSIGN_OR_RAISE(
                            auto message, GetFlatbufMessage(message_obj)
        ...
        ReadFooterAsync(arrow::internal::Executor* executor) {
            const auto magic_size = static_cast<int>(strlen(kArrowMagicBytes));
            ...
            auto read_magic =
                file_->ReadAsync(footer_offset_ - file_end_size, file_end_size);
            if (executor != nullptr) {
            ...
            return read_magic
                .Then(
                    [=](const std::shared_ptr<Buffer>& buffer
                        ...
                        auto read_footer = self->file_->ReadAsync(
                            self->footer_offset_ - footer_length
                                - file_end_size,
                            footer_length
                        );
                        if (executor != nullptr) {
                ...
                .Then([=](const std::shared_ptr<Buffer>& buffer) -> Status {
                    ...
                    const auto* const data = self->footer_buffer_->data();
        ...
        io::RandomAccessFile* file_{NULLPTR};
        IpcReadOptions options_;
        std::vector<bool> field_inclusion_mask_;
    ...
    WholeIpcFileRecordBatchGenerator::operator()() {
        ...
        if (!read_dictionaries_.is_valid()) {
            ...
            read_dictionaries_ = read_messages.Then(
                [=](const std::vector<Result<std::shared_ptr<Message>>>&
                    ...
                    return ReadDictionaries(state.get(), messages);
                }
            );
        }
        ...
        auto read_messages =
            read_dictionaries_.Then([read_message]() { return read_message; });
    ...
    namespace {
        ...
        ReadSparseCOOIndex(
            ...
            RETURN_NOT_OK(
                internal::GetSparseCOOIndexMetadata(sparse_index, &indices_type)
            );
        ...
        ReadSparseCSFIndex(
            ...
            for (int i = 0; i < static_cast<int>(indices_buffers->size());
                ...
                ARROW_ASSIGN_OR_RAISE(
                    indices_data[i],
                    file->ReadAt(
                        indices_buffers->Get(i)->offset(),
                        indices_buffers->Get(i)->length()
                    )
                );
            }

            return SparseCSFIndex::Make(
                indptr_type,
                indices_type,
                indices_size,
                axis_order,
        ...
        ReadSparseTensorMetadata(
            ...
            *out_fb_sparse_tensor = sparse_tensor;
            ...
            if (!bit_util::IsMultipleOf8(buffer->offset())) {
                return Status::Invalid(
                    "Buffer of sparse index data did not "
    ...
    namespace internal {
        ...
        ReadSparseTensorPayload(const IpcPayload& payload) {
            ...
            RETURN_NOT_OK(ReadSparseTensorMetadata(
                *payload.metadata,
                &type,
                &shape,
                &dim_names,
            ...
            RETURN_NOT_OK(CheckSparseTensorBodyBufferCount(
                payload,
                sparse_tensor_format_id,
    ...
    ReadSparseTensor(const Buffer& metadata, io::RandomAccessFile* file) {
        ...
        switch (sparse_tensor_format_id) {
            ...
            case SparseTensorFormat::CSF: {
                ARROW_ASSIGN_OR_RAISE(
                    sparse_index, ReadSparseCSFIndex(sparse_tensor, shape, file)
                );
                return MakeSparseTensorWithSparseCSFIndex(
                    type,
                    shape,
                    dim_names,
                    checked_pointer_cast<SparseCSFIndex>(sparse_index),
                    data
                );
            }
            default:
