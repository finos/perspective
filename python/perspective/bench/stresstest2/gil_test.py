...
def run(is_async):
    ...
    This example uses PerspectiveBenchRunner, which wraps a task function
    ...
    queue.get()
    runner = PerspectiveBenchRunner(session)
    runner.run()


if __name__ == "__main__":
    """To allow your test script to run within the benchmark harness,
    import and create a `PerspectiveBenchRunner`.

    This will allow you to call the script from the command line with
    timing options:

    ```bash
    yarn bench gil_test.py -c10 -r5 # 10 clients, 5 runs of `client_task`/client
    ```
    """
    print("Sync mode")
