...
class Row extends HTMLElement {
    ...
    set type(t) {
        ...
        const filter_dropdown = this.shadowRoot.querySelector(
            "#filter_operator"
        );

        render(
            this._select_template("TYPE_AGGREGATES", type_config.type || type),
            agg_dropdown
        );
        render(
            this._select_template("TYPE_FILTERS", type_config.type || type),
            filter_dropdown
        );
    ...
    choices(choices) {
        const filter_operand = this.shadowRoot.querySelector("#filter_operand");
        const filter_operator = this.shadowRoot.querySelector(
            "#filter_operator"
        );
        const selector = new Awesomplete(filter_operand, {
            ...
            replace: function(text) {
                ...
                if (
                    filter_operator.value === "in" ||
                    filter_operator.value === "not in"
                ) {
        ...
        filter_operand.addEventListener(
    ...
    set filter(f) {
        ...
        if (filter_dropdown.value !== filter.operator) {
            filter_dropdown.value =
                filter.operator ||
                get_type_config(this.getAttribute("type")).filter_operator;
        }
        ...
        if (
            filter_dropdown.value === perspective.FILTER_OPERATORS.isNull ||
    ...
    _update_filter(event) {
        ...
        if (
            filter_operator.value === perspective.FILTER_OPERATORS.isIn ||
        ...
        this.setAttribute(
            "filter",
            JSON.stringify({operator: filter_operator.value, operand: val})
        );
    ...
    _register_ids() {
        ...
        this._filter_operator = this.shadowRoot.querySelector(
            "#filter_operator"
        );
        this._edit_computed_column_button = this.shadowRoot.querySelector(
            "#row_edit"
        );
    }

    _register_callbacks() {
        this._li.addEventListener(
            "dragstart",
            this._set_data_transfer.bind(this)
        );
        this._li.addEventListener("dragend", () => {
            this.dispatchEvent(new CustomEvent("row-dragend"));
        });
        this._visible.addEventListener("mousedown", event =>
            this.dispatchEvent(
                new CustomEvent("visibility-clicked", {detail: event})
            )
        );
        this._row_close.addEventListener("mousedown", event =>
            this.dispatchEvent(
                new CustomEvent("close-clicked", {detail: event})
            )
        );
        this._agg_dropdown.addEventListener("change", event => {
            this.setAttribute("aggregate", this._agg_dropdown.value);
            this.dispatchEvent(
                new CustomEvent("aggregate-selected", {detail: event})
            );
        });
        ...
        const debounced_filter = _.debounce(
            event => this._update_filter(event),
            50
        );
        this._filter_operator.addEventListener("change", () => {
            this._filter_operand.focus();
            this._filter_operator.style.width = get_text_width(
                this._filter_operator.value
            );
            const filter_input = this.shadowRoot.querySelector(
                "#filter_operand"
            );
            filter_input.style.width = get_text_width(
                "" + this._filter_operand.value,
                30
            );
            debounced_filter();
        });
