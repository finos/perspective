...
const add_computed_column = async page => {
    ...
    await page.evaluate(
        element =>
            element.shadowRoot.querySelector("#add-computed-column").click(),
        viewer
    );
    await page.evaluate(element => {
        let com = element.shadowRoot.querySelector(
    ...
    describe.page(
        ...
        () => {
            describe.page(
                ...
                () => {
                    test.capture(
                        "click on add computed column button opens the UI.",
                        async page => {
                            const viewer = await page.$("perspective-viewer");
                            ...
                            await page.evaluate(
                                element =>
                                    element.shadowRoot
                                        .querySelector("#add-computed-column")
                                        .click(),
                                viewer
                            );
                        }
                    ...
                    test.capture(
                        "click on close button closes the UI.",
                        async page => {
                            const viewer = await page.$("perspective-viewer");
                            await page.shadow_click(
                                "perspective-viewer",
                    ...
                    // input column
                    test.capture(
                        "setting a valid column should set it as input.",
                        async page => {
                            const viewer = await page.$("perspective-viewer");
                            await page.shadow_click(
                                "perspective-viewer",
                    ...
                    test.capture(
                        "setting multiple column parameters should set input.",
                        async page => {
                            const viewer = await page.$("perspective-viewer");
                            await page.shadow_click(
                                "perspective-viewer",
                    ...
                    // computation
                    test.capture(
                        "computations should clear input column.",
                        async page => {
                            const viewer = await page.$("perspective-viewer");
                            await page.shadow_click(
                                "perspective-viewer",
            ...
            // save
            test.capture(
                "saving without parameters should fail as button is disabled.",
                async page => {
                    const viewer = await page.$("perspective-viewer");
                    await page.shadow_click(
                        "perspective-viewer",
            ...
            test.capture(
                "saving a computed column should add it to inactive columns.",
                async page => {
            ...
            test.capture(
                "saving a duplicate column should fail with error message.",
                async page => {
                    await add_computed_column(page);
                    const viewer = await page.$("perspective-viewer");
                    await page.evaluate(
            ...
            test.skip("aggregates by computed column.", async page => {
                ...
                await page.evaluate(
                    element =>
                        element.setAttribute("row-pivots", '["Quantity"]'),
                    viewer
                );
                await page.evaluate(
                    element =>
            ...
            test.capture("pivots by computed column.", async page => {
                ...
                const viewer = await page.$("perspective-viewer");
                await page.evaluate(
                    element => element.setAttribute("row-pivots", '["new_cc"]'),
                    viewer
                );
            });
            ...
            test.capture("adds computed column via attribute", async page => {
                ...
                await page.evaluate(
                    element =>
                        element.setAttribute(
                            "computed-columns",
                            '[{"name":"test","func":"month_bucket","inputs":["Order Date"]}]'
                        ),
                    viewer
                );
                ...
                await page.evaluate(
                    element =>
                        element.setAttribute("columns", '["Quantity", "test"]'),
                    viewer
                );
                await page.waitForSelector(
                    "perspective-viewer:not([updating])"
            ...
            test.capture("sorts by computed column.", async page => {
                ...
                await page.evaluate(
                    element =>
                        element.setAttribute("sort", '[["new_cc", "asc"]]'),
                    viewer
                );
            });
            ...
            test.capture(
                "computed column aggregates should persist.",
                async page => {
                    await add_computed_column(page);
                    const viewer = await page.$("perspective-viewer");
                    await page.evaluate(
            ...
            test.capture(
                "user defined aggregates maintained on computed columns",
                async page => {
                    const viewer = await page.$("perspective-viewer");
                    await page.shadow_click(
                        "perspective-viewer",
