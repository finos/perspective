...
export default (adaptee: d3.ScaleBand<string>): MinBandwidth => {
    const minBandwidth: any = (arg) => {
        return adaptee(arg);
    };

    rebindAll(minBandwidth, adaptee);

    minBandwidth.bandwidth = <T extends number | undefined = undefined>(
        ...args: T[]
    ): GetSetReturn<T, number, MinBandwidth> => {
        if (!args.length) {
            return Math.max(adaptee.bandwidth(), MIN_BANDWIDTH) as GetSetReturn<
                T,
                number,
                MinBandwidth
            >;
