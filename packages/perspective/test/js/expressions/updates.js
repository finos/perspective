...
module.exports = perspective => {
    describe("Expression updates", function() {
        it("Scalar-only expressions should respond to appends", async function() {
        ...
        it("Conditional expressions should respond to appends", async function() {
            const table = await perspective.table({
                x: [1.5, 2.5, 3.5, 4.5],
                y: ["A", "B", "C", "D"]
            });
        ...
        it("Scalar-only expressions should respond to partial updates", async function() {
            const table = await perspective.table(
                {
                    x: [1.5, 2.5, 3.5, 4.5],
                    y: ["A", "B", "C", "D"]
        ...
        it("Appends should notify expression column", async function() {
            const table = await perspective.table({
                x: [1, 2, 3, 4],
                y: ["A", "B", "C", "D"]
            });
            ...
            const after = await view.to_columns();
            expect(after['lower("y")']).toEqual(["a", "b", "c", "d", "hello", "world"]);
            view.delete();
            table.delete();
        });
        ...
        it("Appends should notify expression column with multiple columns", async function() {
            ...
            const before = await view.to_columns();
            expect(before['"x" + ("y" + 5.5) / "z"']).toEqual([4.5, 4, 4.5, 5.25]);
            ...
            const after = await view.to_columns();
            expect(after['"x" + ("y" + 5.5) / "z"']).toEqual([4.5, 4, 4.5, 5.25, 6.1, 7.2, 8.3, 9.4]);
            view.delete();
            table.delete();
        });

        it("Partial updates should notify expression column", async function() {
            const table = await perspective.table(
                {
                    x: [1, 2, 3, 4],
                    y: ["A", "B", "C", "D"]
        ...
        it("Dependent column appends with missing columns should notify expression columns", async function() {
            const table = await perspective.table({
                x: [1, 2, 3, 4],
                y: [1.5, 2.5, 3.5, 4.5],
                z: ["a", "b", "c", "d"]
        ...
        it("Dependent column update appends should notify expression columns, pivoted arity 1", async function() {
            const table = await perspective.table({
                x: [1, 2, 3, 4],
                y: ["A", "B", "C", "C"]
            });
        ...
        it("Dependent column appends should notify expression columns, arity 2", async function() {
            ...
            const after = await view.to_columns();
            expect(after['"w" + "x"']).toEqual([2.5, 4.5, 6.5, 8.5, 12.5, 16.5]);
            view.delete();
            table.delete();
        });

        it("Dependent column updates on all column updates should notify expression columns, arity 2", async function() {
        ...
        it("Dependent column appends should notify expression columns on different views, arity 2", async function() {
            const table = await perspective.table(common.int_float_data);
            const view = await table.view({
                expressions: ['"w" + "x"']
            });
        ...
        it("Dependent column appends should notify equivalent expression columns on different views, arity 2", async function() {
            const table = await perspective.table(common.int_float_data);
            const view = await table.view({
                expressions: ['"w" + "x"']
            });
            ...
            const after2 = await view2.to_columns();
            expect(after['"w" + "x"']).toEqual([2.5, 4.5, 6.5, 8.5, 12.5, 16.5]);
            expect(after2['"w" + "x"']).toEqual([2.5, 4.5, 6.5, 8.5, 12.5, 16.5]);
            expect(after2['"w" - "x"']).toEqual([0.5, 0.5, 0.5, 0.5, 8.5, 8.5]);
            view2.delete();
            view.delete();
            table.delete();
        ...
        it("Dependent column updates should notify expression columns on different views, arity 2.", async function() {
            const table = await perspective.table(common.int_float_data, {index: "x"});
            const view = await table.view({
                expressions: ['"w" + "x"']
            });
        ...
        it("Dependent column updates should notify equivalent expression columns on different views, arity 2.", async function() {
            const table = await perspective.table(common.int_float_data, {index: "x"});
            const view = await table.view({
                expressions: ['"w" + "x"']
            });
        ...
        it("Dependent column update with `null` should notify expression columns on different views, arity 2.", async function() {
            const table = await perspective.table(common.int_float_data, {index: "x"});
            const view = await table.view({
                expressions: ['"w" + "x"']
            });
        ...
        it("Dependent column update with `null` should notify equivalent expression columns on different views, arity 2.", async function() {
            const table = await perspective.table(common.int_float_data, {index: "x"});
            const view = await table.view({
                expressions: ['"w" + "x"']
            });
        ...
        it("Updating with `null` should clear the output expression column.", async function() {
            const table = await perspective.table(
                {
                    w: [1.5, 2.5, 3.5, 4.5],
                    x: [1, 2, 3, 4],
        ...
        it("Updating with `undefined` should not clear the output expression column.", async function() {
            const table = await perspective.table(
                {
                    w: [1.5, 2.5, 3.5, 4.5],
                    x: [1, 2, 3, 4],
        ...
        it("Updates on non-dependent columns should not change expression columns.", async function() {
            var meta = {
                w: "float",
                x: "float",
                y: "string",
        ...
        it("multiple expression columns with updates on source columns", async function() {
            const table = await perspective.table(data);
            const view = await table.view({
                expressions: ['"x" * "y"']
            });
        ...
        it("propagate updates to all expression columns", async function() {
            const table = await perspective.table(data, {index: "x"});
        ...
        it("propagate appends to all expression columns", async function() {
            const table = await perspective.table(data);
    ...
    describe("Computed updates with row pivots", function() {
        ...
        it("should update on dependent columns, subtract", async function() {
            ...
            expect(json).toEqual([
                {__ROW_PATH__: [], int: 13, '"int" - "float"': -2.75, __INDEX__: [0, 3, 1, 2]},
                {__ROW_PATH__: [-1.75], int: 3, '"int" - "float"': -1.75, __INDEX__: [2]},
                {__ROW_PATH__: [-1.5], int: 2, '"int" - "float"': -1.5, __INDEX__: [1]},
                {__ROW_PATH__: [-1.25], int: 4, '"int" - "float"': -1.25, __INDEX__: [3]},
                {__ROW_PATH__: [1.75], int: 4, '"int" - "float"': 1.75, __INDEX__: [0]}
            ]);
        ...
        it("should update on dependent columns, multiply", async function() {
            ...
            expect(json).toEqual([
                {__ROW_PATH__: [], int: 13, '"int" * "float"': 51.25, __INDEX__: [0, 3, 2, 1]},
                {__ROW_PATH__: [7], int: 2, '"int" * "float"': 7, __INDEX__: [1]},
                {__ROW_PATH__: [9], int: 4, '"int" * "float"': 9, __INDEX__: [0]},
                {__ROW_PATH__: [14.25], int: 3, '"int" * "float"': 14.25, __INDEX__: [2]},
                {__ROW_PATH__: [21], int: 4, '"int" * "float"': 21, __INDEX__: [3]}
            ]);
        ...
        it("should update on dependent columns, divide", async function() {
            ...
            expect(json).toEqual([
                {__ROW_PATH__: [], int: 13, '"int" / "float"': 3.742690058479532, __INDEX__: [0, 3, 2, 1]},
                {__ROW_PATH__: [0.5714285714285714], int: 2, '"int" / "float"': 0.5714285714285714, __INDEX__: [1]},
                {__ROW_PATH__: [0.631578947368421], int: 3, '"int" / "float"': 0.631578947368421, __INDEX__: [2]},
                {__ROW_PATH__: [0.7619047619047619], int: 4, '"int" / "float"': 0.7619047619047619, __INDEX__: [3]},
                {__ROW_PATH__: [1.7777777777777777], int: 4, '"int" / "float"': 1.7777777777777777, __INDEX__: [0]}
            ]);
    ...
    describe("Partial update with null", function() {
        ...
        it("Undefined should be a no-op on expression columns", async function() {
            const table = await perspective.table(
                [
