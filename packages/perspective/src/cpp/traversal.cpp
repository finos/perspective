...
t_traversal::populate_root_children(const t_stnode_vec& rchildren)
{
    ...
    for (t_stnode_vec::const_iterator iter = rchildren.begin(); iter != rchildren.end(); ++iter)
    {
...
t_traversal::expand_node(t_tvidx exp_idx)
{
    ...
    t_index count = 0;
    for (t_stnode_vec::const_iterator iter = tchildren.begin(); iter != tchildren.end(); ++iter)
    {
...
t_traversal::add_node(
    const t_sortsvec& sortby, const t_uidxvec& indices, t_index insert_level_idx, t_ctx2* ctx2)
{
