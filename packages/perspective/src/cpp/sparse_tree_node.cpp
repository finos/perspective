...
t_stnode::t_stnode(t_uindex idx,
                   t_uindex pidx,
                   ...
                   t_uindex nstrands,
                   t_uindex aggidx)
    : m_idx(idx), m_pidx(pidx), m_depth(depth), m_nstrands(nstrands),
      m_aggidx(aggidx)
{
    m_value.set(value);
...
t_cellinfo::t_cellinfo(t_ptidx idx,
                       t_depth treenum,
                       t_index agg_index,
                       t_uindex ridx,
                       t_uindex cidx)
    : m_idx(idx), m_treenum(treenum), m_agg_index(agg_index),
      m_ridx(ridx), m_cidx(cidx)
{
}
...
operator<<(std::ostream& os, const perspective::t_stnode& node)
{
    os << "t_stnode<"
       ...
       << " value: " << node.m_value
       << " sort_value: " << node.m_sort_value
       << " aggidx: " << node.m_aggidx
       << " nstrands: " << node.m_nstrands << " depth: "
       << static_cast<perspective::t_uindex>(node.m_depth) << ">";
    return os;
}
...
{
    os << "t_cellinfo<idx: " << node.m_idx
       << " treenum: " << node.m_treenum
