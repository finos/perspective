...
template <>
void sort(std::shared_ptr<t_ctx2> ctx2, val j_sortby, val j_column_sortby) {
    auto svec = _get_sort(j_sortby);
    if (svec.size() > 0) {
        ctx2->sort_by(svec);
    }
...
template <>
val get_column_data(std::shared_ptr<t_table> table, std::string colname) {
    val arr = val::array();
 ...
 */
template <typename T>
val get_data(T ctx, std::uint32_t start_row, std::uint32_t end_row, std::uint32_t start_col,
...
template <>
val get_data_two_skip_headers(std::shared_ptr<t_ctx2> ctx, std::uint32_t depth,
