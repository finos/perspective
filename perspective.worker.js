...
module.exports = __webpack_require__.p + "cdecbdf3813cb85e.wasm";
...
for(const callback of this._delete_callbacks){callback();}};/**
     ...
     */function parse_expression_strings(expressions){let validated_expressions=[];const expression_idx_map={};const is_new_format=!Array.isArray(expressions);if(!is_new_format){console.warn("Legacy `expressions` format: "+JSON.stringify(expressions));}for(let expression of is_new_format?Object.keys(expressions):expressions){const name=expression;if(is_new_format){expression=expressions[expression];}// Map of column names to column IDs, so that we generate
...
config["aggregates"]={};config["columns"]=[];for(const agg of _config["aggregate"]){config["aggregates"][agg["column"]]=agg["op"];config["columns"].push(agg["column"]);}}else if(CONFIG_VALID_KEYS.indexOf(key)>-1){config[key]=_config[key];}else{throw new Error(`Unrecognized config parameter "${key}"`);}}config.group_by=config.group_by||[];config.split_by=config.split_by||[];config.aggregates=config.aggregates||{};config.filter=config.filter||[];config.sort=config.sort||[];config.expressions=config.expressions||{};const table_schema=this.schema();if(config.expressions!==undefined){config.expressions=parse_expression_strings(config.expressions);}if(config.columns===undefined){// If columns are not provided, use all columns
