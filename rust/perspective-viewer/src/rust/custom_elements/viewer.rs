...
impl PerspectiveViewerElement {
        ...
        ApiFuture::spawn({
            ...
            async move {
                if let Some(theme_name) = theme.get_name().await {
                    elem.set_attribute("theme", &theme_name)?;
                }
    ...
    pub fn js_delete(&mut self) -> ApiFuture<bool> {
        ...
        ApiFuture::from(self.renderer.clone().with_lock(async move {
            renderer.delete()?;
    ...
    pub fn js_resize(&self, force: bool) -> ApiFuture<()> {
        ...
        ApiFuture::from(async move { renderer.resize().await })
    }
